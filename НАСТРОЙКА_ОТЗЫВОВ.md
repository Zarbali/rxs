...
1. В Supabase **Redirect URLs** должен быть **точный** URL страницы отзывов, например: `https://username.github.io/rxs/reviews.html`
2. В Discord Developer Portal → OAuth2 → Redirects должен быть только: `https://hkvmbhrhzitumvtgwzyq.supabase.co/auth/v1/callback` (НЕ URL вашего сайта)
3. Пользователи Supabase Auth создаются автоматически при первом входе через Discord в разделе **Authentication** → **Users**
